...
if [ -f "${SPARK_APPLICATION_JAR_LOCATION}" ]; then
    ...
    /spark/bin/spark-submit \
        ...
        --master ${SPARK_MASTER_NAME} \
        ${SPARK_SUBMIT_ARGS} \
        ${SPARK_APPLICATION_JAR_LOCATION} ${SPARK_APPLICATION_ARGS}
else
    if [ -f "${SPARK_APPLICATION_PYTHON_LOCATION}" ]; then
        echo "Submit application ${SPARK_APPLICATION_PYTHON_LOCATION} to Spark master ${SPARK_MASTER_NAME}"
        echo "Passing arguments ${SPARK_APPLICATION_ARGS}"
        PYSPARK_PYTHON=python2 /spark/bin/spark-submit \
            --master ${SPARK_MASTER_NAME} \
            ${SPARK_APPLICATION_PYTHON_LOCATION} ${SPARK_APPLICATION_ARGS}
        exit $?
    else
        echo "Python application location not founded"
        exit 1
    fi
fi